...
## From https://github.com/popcar2/GodotTilemapBaker/blob/main/TilemapCollisionBaker.gd
##
## How it works TLDR:
...
func run_code(_fake_bool = null):
	...
	var unfiltered_tilemap_locations = tile_map.get_used_cells(target_tiles_layer)
	var tilemap_locations = []
	
	for pos in unfiltered_tilemap_locations:
		var data = tile_map.get_cell_tile_data(target_tiles_layer, pos)
		if data.get_collision_polygons_count(0) > 0:
			tilemap_locations.append(pos)
